...
public class BiomeDecorator
{
    ...
    public BiomeDecorator(BiomeGenBase par1BiomeGenBase)
    {
        ...
        this.ironGen = new WorldGenMinable(Block.oreIron.blockID, 12);
    ...
    {
    	int varx = this.chunk_X; //chunk X
    	int varz = this.chunk_Z; //chunk Z
    	int spawnx = this.currentWorld.getSpawnPoint().posX; //world spawn x
    	int spawny = this.currentWorld.getSpawnPoint().posY; //world spawn z
    	double[] coal_clustersize = BOCSettings.coal_clustersize;
		int[] coal_rarity = BOCSettings.coal_rarity;
		int coal_variation = BOCSettings.coal_variation;
		int[] coal_biomes = BOCSettings.coal_biomes;
		double[] iron_clustersize = BOCSettings.iron_clustersize;
		int[] iron_rarity = BOCSettings.iron_rarity;
		int iron_variation = BOCSettings.iron_variation;
		int[] iron_biomes = BOCSettings.iron_biomes;
    	double coal_clump = (BOCWorldGenOreLakes.CheckRadius(this.currentWorld, varx, varz)) == true ? coal_clustersize[0] : coal_clustersize[1];
    	double iron_clump = (BOCWorldGenOreLakes.CheckRadius(this.currentWorld, varx, varz)) == true ? iron_clustersize[0] : iron_clustersize[1];
        int coal_rare = (BOCWorldGenOreLakes.CheckRadius(this.currentWorld, varx, varz)) == true ? coal_rarity[0] : coal_rarity[1];
        int iron_rare =(BOCWorldGenOreLakes.CheckRadius(this.currentWorld, varx, varz)) == true ? iron_rarity[0] : iron_rarity[1];
        ...
        this.genStandardOre1(3, this.redstoneGen, 0, 16);
        this.genStandardOre1(1, this.diamondGen, 0, 16);
        this.genStandardOre2(1, this.lapisGen, 16, 16);
        
        if (this.randomGenerator.nextInt(coal_rare) == 0 && (BOCWorldGenOreLakes.contains(coal_biomes, var99) || BOCWorldGenOreLakes.contains(coal_biomes, -1))) //Chance inside which biome default:4
        {
        	int var12 = varx + this.randomGenerator.nextInt(16) + 8;
        	int var13 = this.randomGenerator.nextInt(150);//this.rand.nextInt(128); //from what Y level and down it will spawn ores
        	int var14 = varz + this.randomGenerator.nextInt(16) + 8;
        	
        	(new BOCWorldGenOreLakes(Block.oreCoal.blockID, coal_clump, coal_variation)).generate(this.currentWorld, this.randomGenerator, var12, var13, var14);
        }
        if (this.randomGenerator.nextInt(iron_rare) == 0 && (BOCWorldGenOreLakes.contains(iron_biomes, var99) || BOCWorldGenOreLakes.contains(iron_biomes, -1))) //Chance inside which biome default:4
        {
        	int var12 = varx + this.randomGenerator.nextInt(16) + 8;
        	int var13 = this.randomGenerator.nextInt(130);//this.rand.nextInt(128); //from what Y level and down it will spawn ores
        	int var14 = varz + this.randomGenerator.nextInt(16) + 8;
        	
        	(new BOCWorldGenOreLakes(Block.oreIron.blockID, iron_clump, iron_variation)).generate(this.currentWorld, this.randomGenerator, var12, var13, var14);
        }
    }
}
